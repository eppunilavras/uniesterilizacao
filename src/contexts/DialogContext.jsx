...
export function DialogProvider({ children }) {
  ...
  const getButtonStyles = (variant) => {
      switch(variant) {
          ...
          default: return 'bg-[#021D34] hover:bg-[#009DE0] text-white shadow-blue-900/20';
      }
  };
  ...
  return (
    ...
    <DialogContext.Provider value={{ confirm, alert }}>
      {children}
      {dialog && (
        <div className="fixed inset-0 z-[10050] flex items-center justify-center p-4 bg-[#021D34]/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div 
            className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100 transition-colors"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="p-5 border-b border-slate-100 flex gap-4 items-start bg-slate-50 transition-colors">
                <div className={`p-2 rounded-full shrink-0 ${dialog.variant === 'destructive' ? 'bg-red-100' : 'bg-amber-100'}`}>
                    {getIcon(dialog.variant)}
                </div>
                <div className="flex-1">
                    <h3 className="font-bold text-lg text-[#021D34] leading-tight mb-1">
                        {dialog.title}
                    </h3>
            ...
            {/* Footer Actions */}
            <div className="p-4 bg-slate-50 flex gap-3 transition-colors">
                {dialog.cancelLabel && (
                    <button 
                        onClick={dialog.onCancel}
                        className="flex-1 px-4 py-2.5 rounded-xl font-bold text-sm text-slate-600 bg-white border border-slate-200 hover:bg-slate-100 transition-all"
                    >
                        {dialog.cancelLabel}
                    </button>
                )}
