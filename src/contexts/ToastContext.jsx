...
export function ToastProvider({ children }) {
  ...
  const getStyles = (type) => {
    switch (type) {
      case 'success': return 'border-l-4 border-l-green-500 bg-white';
      case 'error': return 'border-l-4 border-l-red-500 bg-white';
      case 'warning': return 'border-l-4 border-l-amber-500 bg-white';
      default: return 'border-l-4 border-l-blue-500 bg-white';
    }
  };
      ...
      <div className="no-print fixed top-4 right-4 z-[10060] flex flex-col gap-2 pointer-events-none">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`
                pointer-events-auto w-80 p-4 rounded-lg shadow-xl shadow-slate-200/50 
                flex items-start gap-3 transform transition-all duration-300 animate-in slide-in-from-right-full 
                border border-slate-100 ${getStyles(toast.type)}
            `}
          >
            <div className="shrink-0 mt-0.5">{getIcon(toast.type)}</div>
            <div className="flex-1 min-w-0">
                {toast.title && <h4 className="font-bold text-sm text-slate-800 mb-0.5">{toast.title}</h4>}
                <p className={`text-sm font-medium leading-tight ${toast.title ? 'text-slate-600' : 'text-slate-800 font-semibold'}`}>
                    {toast.message}
                </p>
            </div>
            <button
