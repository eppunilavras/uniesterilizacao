...
const StatCard = ({ title, count, icon: Icon, color, bg }) => {
    return (
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:translate-y-[-2px] transition-all duration-300">
            <div className="flex justify-between items-start mb-4">
                {/* O container do ícone usa as classes passadas via props (bg e color).
                   ...
                   no componente pai (ex: bg="bg-orange-50").
                */}
                <div className={`p-3 rounded-xl transition-colors ${bg} ${color}`}>
                    <Icon size={24}/>
                </div>
            </div>
            <h3 className="text-3xl font-bold text-[#021D34] transition-colors">
                {count}
            </h3>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mt-1 transition-colors">
                {title}
            </p>
        </div>
    );
