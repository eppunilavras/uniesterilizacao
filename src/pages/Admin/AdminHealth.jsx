...
export default function AdminHealth() {
  ...
  const {
    ...
    queryFn: async () => {
      const collections = [
        {
          id: "items",
          ref: collection(db, "artifacts", appId, "public", "data", "items"),
        },
        {
          id: "users",
          ref: collection(
  ...
  const metrics = isFetched
    ? [
        ...
        {
          label: "Recados Publicados",
          value: health.announcements,
          icon: Megaphone,
          color: "text-pink-500",
          bg: "bg-pink-50",
        },
  ...
  if (!isFetched && !isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 transition-colors">
        <div className="bg-white p-4 rounded-full shadow-sm mb-4">
          <Database className="text-[#009DE0] opacity-40" size={48} />
        </div>
        <h3 className="text-[#021D34] font-bold text-lg mb-2">
          Monitorização de Dados
        </h3>
        <p className="text-slate-500 text-sm max-w-xs text-center mb-6">
          As métricas de volume do sistema não são carregadas automaticamente
          para poupar créditos de leitura do Firebase.
        </p>
        <button
          onClick={() => refetch()}
          className="bg-[#009DE0] hover:bg-[#008bc5] text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-blue-200 transition-all active:scale-95"
        >
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        ...
        <p className="text-sm font-bold animate-pulse">
          A consultar o servidor Firebase...
        </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-8 bg-red-50 border border-red-100 rounded-xl text-red-600 flex flex-col items-center gap-3">
        <AlertCircle size={32} />
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {metrics.map((m, i) => (
          ...
          >
            ...
            <span className="block text-3xl font-bold text-[#021D34] tabular-nums tracking-tight">
      ...
      <div className="bg-blue-50 border border-blue-100 p-5 rounded-2xl flex gap-4 transition-colors">
        <Database className="text-blue-500 shrink-0" size={24} />
        <div className="space-y-1">
          <p className="font-bold text-blue-900 text-sm">
            Nota sobre o consumo
          </p>
          <p className="text-blue-800/70 text-xs leading-relaxed">
            Estas métricas representam o volume de documentos no Firestore.
            Leituras e escritas em tempo real (custos de faturamento) devem ser
            consultadas diretamente no <strong>Console do Firebase</strong>.
          </p>
        </div>
      </div>
    </div>
