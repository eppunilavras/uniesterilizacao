...
export default function AdminData() {
  ...
  const reviveTimestamps = (obj) => {
    ...
    return Object.fromEntries(
      Object.entries(obj).map(([k, v]) => [k, reviveTimestamps(v)]),
    );
  };
  ...
  const performBackup = async () => {
    ...
    for (const user of userDir) {
      try {
        const snap = await getDoc(
          doc(db, "artifacts", appId, "users", user.id, "profile", "data"),
        );
        if (snap.exists()) {
          profiles.push({ uid: user.id, ...snap.data() });
        }
      } catch (e) {
        console.warn(`Falha ao pegar perfil de ${user.id}`, e);
      }
    }
    ...
    a.href = url;
    a.download = `backup_unilavras_${timestamp}.json`;
    a.click();
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!rawJSON.items && !rawJSON.users_directory)
        ...
        setRestorePreview({
          ...
          date: rawJSON.metadata?.exportedAt
            ? new Date(rawJSON.metadata.exportedAt).toLocaleString()
            : "Desconhecida",
          counts: {
  ...
  const executeRestore = async () => {
    ...
    try {
      ...
      for (const key of collectionsToRestore) {
        if (data[key]) {
          for (const item of data[key]) {
            ...
            if (id) {
              currentBatch.set(
                doc(db, "artifacts", appId, "public", "data", key, id),
                rest,
              );
              operationCount++;
              totalItemsProcessed++;
      ...
      await logEvent(
        "DATA_OP",
        `Restauração realizada: ${restorePreview.fileName}`,
      );
      addToast(
        `Sucesso! ${totalItemsProcessed} registros restaurados.`,
        "success",
      );
    ...
    try {
        ...
        message: (
          <div className="space-y-3">
            <p className="text-red-600 font-bold">
              ⚠️ Verifique sua pasta de Downloads!
            </p>
            <p>
              O arquivo JSON foi salvo? Se você não tiver o arquivo, NÃO
              prossiga.
            </p>
      ...
      for (const colName of collectionsToDelete) {
        ...
        for (const [index, chunk] of chunks.entries()) {
          setStatusMsg(
            `Apagando ${colName}: lote ${index + 1}/${chunks.length}...`,
          );
      ...
      await logEvent(
        "DATA_OP",
        `RESET TOTAL REALIZADO. ${totalDeleted} removidos.`,
      );
      addToast(
        `Limpeza concluída! ${totalDeleted} registros apagados.`,
        "success",
      );
    } catch (e) {
  ...
  const requestWipe = async () => {
    ...
    if (
      !(await confirm({
        ...
        message:
          "Você solicitou a limpeza total. Vamos gerar um backup OBRIGATÓRIO antes. Continuar?",
  ...
  const getModalDescription = () => {
    if (pendingAction === "BACKUP")
      return "Para exportar os dados sensíveis do sistema, confirme sua senha.";
    if (pendingAction === "RESTORE")
      return "Para sobrescrever o banco de dados com o backup, confirme sua senha.";
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in">
            ...
            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
              <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl transition-colors">
                ...
                <div className="flex gap-4 mt-2 text-sm text-slate-600">
                  ...
                  <span className="flex items-center gap-1">
                    <Calendar size={14} /> {restorePreview.date}
                  </span>
                </div>
              </div>
              ...
              {restorePreview.counts.items > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(
                      restorePreview.detailedStats.statusCounts,
                    ).map(([status, count]) => {
                      ...
                      return (
                        <div
                          key={status}
                          className={`flex justify-between items-center p-2 rounded border text-xs font-bold uppercase ${config.color}`}
                        >
              ...
              <div className="p-4 bg-yellow-50 border border-yellow-100 rounded-xl flex gap-3 text-yellow-800 text-sm transition-colors">
                ...
                <p>
                  ...
                  <strong>substituídos</strong>. Será solicitada sua senha.
                </p>
              </div>
            </div>
      ...
      <div className="bg-red-50 p-6 rounded-xl border border-red-200 flex flex-col justify-center relative overflow-hidden transition-colors">
        ...
        <p className="text-sm text-red-700 mb-6 leading-relaxed">
          ...
          <br />
          <br />
          <span className="font-bold">O que será APAGADO:</span> Itens,
          Históricos, Logs e Avisos.
          <br />
          <span className="font-bold">O que será MANTIDO:</span> Usuários, Tipos
          e Configurações.
        </p>
        ...
        <button
          onClick={requestWipe}
          disabled={loading}
          className="w-full bg-red-600 text-white py-4 rounded-xl font-bold hover:bg-red-700 disabled:opacity-50 shadow-lg shadow-red-900/20 flex items-center justify-center gap-2 transition-colors"
        >
