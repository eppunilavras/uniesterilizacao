...
export default function AdminAnnouncements() {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in">
            {/* Modal de Preview */}
            {previewItem && (
                <div className="fixed inset-0 z-[10002] flex items-center justify-center p-4 bg-[#021D34]/50 backdrop-blur-sm animate-in fade-in" onClick={() => setPreviewItem(null)}>
                    <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-sm w-full animate-in zoom-in-95 transition-colors" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-[#021D34]">Pré-visualização</h3>
                            <button onClick={() => setPreviewItem(null)} className="p-1 hover:bg-slate-100 text-slate-500 rounded transition-colors"><X size={20}/></button>
                        </div>
            ...
            {/* Formulário */}
            <form onSubmit={save} className="space-y-4 border-r-0 md:border-r md:pr-6 border-slate-100 order-last md:order-first transition-colors">
                ...
                <div className="space-y-1">
                    ...
                    <input 
                        className="w-full p-3 border rounded-lg text-sm bg-white border-slate-200 text-slate-900 outline-none focus:ring-1 focus:ring-[#009DE0] transition-colors" 
                ...
                <div className="space-y-1">
                    ...
                    <textarea 
                        className="w-full p-3 border rounded-lg text-sm h-32 bg-white border-slate-200 text-slate-900 outline-none focus:ring-1 focus:ring-[#009DE0] transition-colors resize-none" 
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                        ...
                        <input 
                            type="datetime-local" 
                            className="w-full p-3 border rounded-lg text-xs bg-white border-slate-200 text-slate-900 outline-none focus:ring-1 focus:ring-[#009DE0] transition-colors" 
                ...
                <div className="flex gap-2 pt-2">
                    {editingId && (
                        <button 
                            type="button" 
                            onClick={cancelEdit} 
                            className="flex-1 bg-slate-100 text-slate-600 p-3 rounded-lg font-bold hover:bg-slate-200 transition-colors"
                        >
                            Cancelar
                        </button>
                    )}
                    <button className="flex-1 bg-[#021D34] text-white p-3 rounded-lg font-bold hover:bg-[#009DE0] transition-colors shadow-lg active:scale-95">
            ...
            <div className="md:col-span-2 space-y-4">
                ...
                {anns.map(a => (
                    <div 
                        ...
                        ${editingId === a.id 
                            ? 'bg-blue-50 border-[#009DE0] ring-1 ring-blue-100' 
                    ...
                    >
                        ...
                        <div className="flex gap-1 self-end md:self-start ml-4">
                            <button 
                                ...
                                className="text-slate-400 hover:text-[#009DE0] p-2 hover:bg-white rounded-lg transition-colors" 
                            ...
                            <button 
                                ...
                                className="text-blue-400 hover:text-blue-600 p-2 hover:bg-white rounded-lg transition-colors" 
                                title="Editar"
                            >
                                <Edit2 size={16}/>
                            </button>
                            <button 
                                onClick={() => remove(a.id)} 
                                className="text-red-400 hover:text-red-600 p-2 hover:bg-white rounded-lg transition-colors" 
                                title="Excluir"
                            >
                                <Trash2 size={16}/>
                            </button>
