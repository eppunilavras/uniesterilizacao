    ...
    if (isMobileBlock) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center animate-in zoom-in-95">
                <div className="bg-blue-50 p-6 rounded-full mb-6 transition-colors"><Monitor className="w-16 h-16 text-[#009DE0]" /></div>
                <h2 className="text-2xl font-bold text-[#021D34] mb-3 transition-colors">Acesso Restrito</h2>
                <p className="text-slate-500 max-w-md mx-auto transition-colors">Acesse esta página em um computador.</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 items-start w-full max-w-full overflow-x-hidden animate-in fade-in">
            ...
            <div className="w-full lg:flex-1 space-y-6 order-2 lg:order-1 min-w-0">
                ...
                <div className="flex bg-slate-200 p-1 rounded-lg w-full overflow-x-auto no-scrollbar touch-pan-x max-w-[85vw] lg:max-w-full transition-colors">
                    {[
                    ...
                    {activeTab === 'layout' && (
                        <div className="space-y-5 animate-in fade-in">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 mb-1 block">Largura (mm)</label>
                                    <input type="number" className="w-full p-2 border rounded text-sm bg-white border-slate-200 text-slate-900" value={settings.width} onChange={e=>setSettings({...settings, width: Number(e.target.value)})}/>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 mb-1 block">Altura (mm)</label>
                                    <input type="number" className="w-full p-2 border rounded text-sm bg-white border-slate-200 text-slate-900" value={settings.height} onChange={e=>setSettings({...settings, height: Number(e.target.value)})}/>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 mb-1 block">Margem (mm)</label>
                                    <input type="number" className="w-full p-2 border rounded text-sm bg-white border-slate-200 text-slate-900" value={settings.margin} onChange={e=>setSettings({...settings, margin: Number(e.target.value)})}/>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-500 mb-1 block">Orientação</label>
                                    <select className="w-full p-2 border rounded text-sm bg-white border-slate-200 text-slate-900" value={settings.orientation} onChange={e=>setSettings({...settings, orientation: e.target.value})}>
                            ...
                            <div className={`space-y-6 transition-opacity duration-300 ${settings.showHeader ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                    ...
                                    <div className="flex border rounded overflow-hidden bg-white">
                                        {['flex-start', 'center', 'flex-end'].map((align, idx) => (
                                            <button 
                                                ...
                                                ${settings.headerAlign === align ? 'bg-[#009DE0] text-white' : 'text-slate-400'}`}
                                            >
                                ...
                                {settings.showTitle && (
                                    <div className="p-3 border rounded-lg bg-slate-50/50">
                                        ...
                                        <input className="w-full p-2 border rounded text-xs mb-3 bg-white border-slate-200 text-slate-900" value={settings.customTitle} onChange={e => setSettings({...settings, customTitle: e.target.value})} placeholder="Texto do Título"/>
                                        <div className="space-y-3">
                                            <div><div className="flex justify-between"><label className="text-[10px] font-bold text-slate-500">Tamanho Fonte</label><span className="text-[10px] text-slate-400">{settings.headerFontSize}px</span></div><input type="range" min="6" max="32" className="w-full accent-[#009DE0]" value={settings.headerFontSize} onChange={e => setSettings({...settings, headerFontSize: Number(e.target.value)})}/></div>
                                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {settings.showTitle && (
                                    <div className="p-3 border rounded-lg bg-slate-50/50">
                                        ...
                                        <input className="w-full p-2 border rounded text-xs mb-3 bg-white border-slate-200 text-slate-900" value={settings.customSubtitle} onChange={e => setSettings({...settings, customSubtitle: e.target.value})} placeholder="Texto do Subtítulo"/>
                                        <div className="space-y-3">
                                            <div><div className="flex justify-between"><label className="text-[10px] font-bold text-slate-500">Tamanho Fonte</label><span className="text-[10px] text-slate-400">{settings.subheaderFontSize}px</span></div><input type="range" min="4" max="32" className="w-full accent-[#009DE0]" value={settings.subheaderFontSize} onChange={e => setSettings({...settings, subheaderFontSize: Number(e.target.value)})}/></div>
                                            <div className="grid grid-cols-2 gap-4">
                                ...
                                {settings.showDate && (
                                    <div className="p-3 border rounded-lg bg-slate-50/50">
                                        ...
                                        <div className="space-y-3">
                                            <div><div className="flex justify-between"><label className="text-[10px] font-bold text-slate-500">Tamanho Fonte</label><span className="text-[10px] text-slate-400">{settings.dateFontSize}px</span></div><input type="range" min="4" max="32" className="w-full accent-[#009DE0]" value={settings.dateFontSize} onChange={e => setSettings({...settings, dateFontSize: Number(e.target.value)})}/></div>
                                            <div className="grid grid-cols-2 gap-4">
                    ...
                    {activeTab === 'footer' && (
                        <div className="space-y-5 animate-in fade-in">
                            ...
                            <div className={`space-y-4 transition-opacity ${settings.showFooter ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                <div><div className="flex justify-between mb-1"><label className="text-xs font-bold text-slate-500">Tamanho Texto</label></div><input type="range" min="6" max="32" className="w-full accent-[#009DE0]" value={settings.footerFontSize} onChange={e => setSettings({...settings, footerFontSize: Number(e.target.value)})} /></div>
                                ...
                                <div className="pt-2">
                                    <label className="text-xs font-bold text-slate-500 mb-2 block">Alinhamento Texto</label>
                                    <div className="flex border rounded-lg overflow-hidden bg-slate-50">
            ...
            <div className="w-full lg:w-[400px] xl:w-[450px] shrink-0 order-1 lg:order-2 lg:sticky lg:top-4 mx-auto">
                ...
                <div className="bg-slate-100 p-6 rounded-xl border border-slate-300 flex flex-col items-center justify-center min-h-[300px] md:min-h-[400px] relative overflow-hidden transition-colors">
