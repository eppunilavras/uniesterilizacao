...
export default function UserManagement({ userProfile }) {
    ...
    const checkCpfExists = async (cpf, excludeUid = null) => {
        const q = query(collection(db, 'artifacts', appId, 'public', 'data', 'users_directory'), where('cpf', '==', cpf));
    ...
    const saveUser = async (e) => {
        ...
        try {
            if (editing) {
                ...
                if (await checkCpfExists(form.cpf, editing.uid)) {
                ...
                await logEvent('USER_MGMT', `Usuário atualizado: ${form.name}`, { uid: editing.uid, changes: updates, executor: userProfile.email });
                addToast('Dados atualizados!', 'success');
            } else {
                if (await checkCpfExists(form.cpf)) {
                ...
                const data = { 
                    name: form.name, email: form.email, cpf: form.cpf, role: form.role, active: true, createdAt: serverTimestamp() 
                };
                ...
                await logEvent('USER_MGMT', `Novo usuário cadastrado: ${form.name}`, { email: form.email, role: form.role, executor: userProfile.email });
    ...
    const executeImport = async () => {
        ...
        for (const user of toImport) {
            ...
            if (await checkCpfExists(user.cpf)) {
                errors++;
                errorDetails.push(`${user.name}: CPF ${user.cpf} já cadastrado.`);
                continue;
            }
        ...
        if (errors > 0) {
                ...
                message: (
                    <div className="space-y-4">
                        ...
                        <div className="bg-slate-50 border border-slate-200 rounded-lg overflow-hidden">
                            <div className="bg-slate-100 px-3 py-2 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">
                                Detalhes dos Erros
                            </div>
                            <div className="max-h-48 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                                {errorDetails.map((err, i) => (
                                    <div key={i} className="text-xs flex gap-2 items-start text-slate-600 border-b border-slate-100 last:border-0 pb-1 last:pb-0">
            ...
            addToast(`${success} usuários importados com sucesso!`, 'success');
        }
        fetchUsers(search);
    };
    ...
    const toggleAll = () => {
        if (selectedImportIndices.size === csvPreview.length) setSelectedImportIndices(new Set());
        else setSelectedImportIndices(new Set(csvPreview.map(i => i.id)));
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        ...
                        <button onClick={() => { setView('form'); setEditing(null); setForm({ name: '', email: '', cpf: '', role: 'student', password: '', active: true }); }} className={`flex-1 px-4 py-2 rounded-lg text-sm font-bold ${view === 'form' ? 'bg-[#021D34] text-white' : 'bg-white border'}`}>Novo</button>
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[10005] flex items-center justify-center p-4 bg-[#021D34]/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <div>
                                <h3 className="text-xl font-bold text-[#021D34]">Confirmar Importação</h3>
                                <p className="text-sm text-slate-500">Selecione os usuários que deseja cadastrar.</p>
                            </div>
                                    ...
                                    <tr>
                                        <th className="p-4 w-12 text-center">
                                            <button onClick={toggleAll} className="text-[#009DE0]">
                                                {selectedImportIndices.size === csvPreview.length ? <CheckSquare size={20}/> : <Square size={20}/>}
                                            </button>
                                        </th>
                                    ...
                                    {csvPreview.map((item) => (
                                        <tr key={item.id} className={`transition-colors ${selectedImportIndices.has(item.id) ? 'bg-white' : 'bg-slate-100 opacity-60'}`}>
                                            <td className="p-4 text-center">
                                                <button onClick={() => toggleSelect(item.id)} className={`${selectedImportIndices.has(item.id) ? 'text-[#009DE0]' : 'text-slate-400'}`}>
                                                    {selectedImportIndices.has(item.id) ? <CheckSquare size={20}/> : <Square size={20}/>}
                                                </button>
                                            </td>
                                            <td className="p-4 font-medium text-[#021D34]">{item.name}</td>
                                            <td className="p-4 text-slate-600">{item.email}</td>
                                            <td className="p-4">
                                                <span className={`px-2 py-1 rounded text-[10px] uppercase font-bold ${item.role === 'admin' ? 'bg-purple-100 text-purple-700' : item.role === 'tech' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                        ...
                        <div className="p-4 border-t border-slate-100 bg-white flex flex-col md:flex-row justify-between items-center gap-4 rounded-b-2xl">
                            <div className="text-sm">
                                <span className="font-bold text-[#009DE0]">{selectedImportIndices.size}</span> selecionados de <span className="font-bold">{csvPreview.length}</span>.
                                {importStats.ignored > 0 && <span className="ml-2 text-orange-600 text-xs flex items-center gap-1 inline-flex"><AlertTriangle size={12}/> {importStats.ignored} ignorados (permissão).</span>}
                            </div>
