...
export default function UserManagement({ userProfile }) {
  ...
  const formatSearchTerm = (text) => {
    ...
    return trimmed.toLowerCase().replace(/(?:^|\s)\S/g, function (a) {
      ...
      if (editing) {
        if (!data.active) {
          const q = query(
            ...
            where("studentId", "==", editing.uid),
            where("status", "!=", "retirado"),
          );
          const snap = await getDocs(q);
          if (!snap.empty) {
            addToast(
              `Não é possível inativar. Utilizador tem itens pendentes.`,
              "error",
            );
            return;
          }
        ...
        if (editing.role !== data.role)
          changesDetected.push(`Perfil: ${editing.role} -> ${data.role}`);
        if (editing.active !== data.active)
          changesDetected.push(
            `Status: ${editing.active ? "Ativo" : "Inativo"} -> ${data.active ? "Ativo" : "Inativo"}`,
          );
        if (editing.name !== data.name) changesDetected.push(`Nome alterado`);
        if (editing.email !== data.email)
          changesDetected.push(`Email alterado`);
        if (editing.cpf !== cleanCpf) changesDetected.push(`CPF alterado`);
        // ------------------------------------------------

        batch.update(
          doc(db, "artifacts", appId, "users", editing.uid, "profile", "data"),
          updates,
        );
        batch.update(
          doc(
        ...
        await logEvent("USER_MGMT", `Utilizador atualizado: ${data.name}`, {
          ...
          updates: updates,
          changesSummary:
            changesDetected.join(", ") || "Atualização de dados cadastrais",
  ...
  const handleInactivate = async (u) => {
    if (u.uid === userProfile.uid) {
      addToast("Não pode inativar a si mesmo.", "error");
      return;
    }
    ...
    try {
      ...
      batch.update(
        doc(db, "artifacts", appId, "users", u.uid, "profile", "data"),
        { active: false },
      );
      batch.update(
        doc(db, "artifacts", appId, "public", "data", "users_directory", u.uid),
        { active: false },
      );
      await batch.commit();
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (event) => {
      ...
      if (parsedData.length === 0) {
        addToast(
          ignoredCount > 0 ? "Nenhum utilizador válido." : "Arquivo inválido.",
          "error",
        );
        e.target.value = "";
        return;
  ...
  const toggleAll = () => {
    if (selectedImportIndices.size === csvPreview.length)
                  ...
                  <tr>
                    <th className="p-4 w-12 text-center">
                      <button onClick={toggleAll} className="text-[#009DE0]">
                        {selectedImportIndices.size === csvPreview.length ? (
                          <CheckSquare size={20} />
                        ) : (
                          <Square size={20} />
                        )}
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col md:flex-row justify-between items-center gap-4 rounded-b-2xl transition-colors">
              ...
              <div className="flex gap-3 w-full md:w-auto">
                ...
                <button
                  ...
                  disabled={importing || selectedImportIndices.size === 0}
                  className="flex-1 md:flex-none px-6 py-3 bg-[#021D34] text-white rounded-xl font-bold hover:bg-[#009DE0] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors shadow-lg shadow-blue-900/20"
                >
                  {importing ? (
                    <Loader2 className="animate-spin w-5 h-5" />
                  ) : (
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-colors">
          ...
          {search.length > 2 && (
            <div className="text-xs text-slate-400 px-2 mb-2">
              A pesquisar por:{" "}
              <span className="font-bold">"{formatSearchTerm(search)}"</span>
            </div>
          )}
          <DataTable
            columns={[
            ...
            data={users}
            emptyMsg={
              loading ? "A pesquisar..." : "Nenhum utilizador encontrado."
            }
            mobileRender={(u) => (
            ...
            actions={(u) => {
              const canEdit =
                userProfile.role === "admin" ||
                (userProfile.role === "tech" && u.role === "student");
