...
export default function LoginScreen({ globalError }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const userDoc = await getDoc(
        doc(db, "artifacts", appId, "users", cred.user.uid, "profile", "data"),
      );
  ...
  const handleSubstituteLogin = async (e) => {
    ...
    try {
      ...
      await logEvent(
        "LOGIN",
        "Acesso via código de substituição",
        { code, label: codeData.label },
        { uid: "substituto", email: SUBSTITUTO_EMAIL },
      );
    } catch (error) {
      addToast(translateFirebaseError(error), "error");
      setIsSubmitting(false);
          ...
          <form onSubmit={handleSubstituteLogin} className="space-y-4">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <ShieldCheck size={18} /> Entrar com Código
                </>
              )}
            </button>
          </form>
